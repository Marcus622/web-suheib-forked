...
app.get("/tiere/:id", (req, res) => {
    ...
    const selectIdTiereQuery = "SELECT * FROM tiere WHERE id = ?";

    db.get(selectIdTiereQuery, [id], (err, row) => {
        if (err) {
            res.status(500).send("Fehler in deiner Query Anfrage");
        } else if (!row) {
            res.status(404).send("Kein Tier mit dieser ID gefunden.");
        }
        else {
            res.json(row);
        }
        ...
        app.put("/tiere/:id", (req, res) => {
            const id = req.params.id;
            const { tierart, name, krankheit, age, gewicht } = req.body
            db.run(`UPDATE tiere SET tierart=?,name=?,krankheit=?,age=?,gewicht=?  WHERE id = ?`, [tierart, name, krankheit, age, gewicht,id])
            res.status(201).send("Tier wurde erfolgreich geändert")
